...
template <class T>
MatDoub rtbisM(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	...
	for (Int j=0;j<JMAX;j++) {
		...
		bisTable[j][5] = abs(dx);
		if (fmid <= 0.0) rtb=xmid;
		if (abs(dx) < xacc || fmid == 0.0) return bisTable;
...
template <class T>
MatDoub rtsecM(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	const Int MAXIT=30;
	...
	Doub f=func(x2);
	Doub temp;
	Doub C = 0.4; //Convergence constant
	if (abs(fl) < abs(f)) {
		rts=x1;
		xl=x2;
		SWAP(fl,f);
	...
	for (Int j=0;j<MAXIT;j++) {
		...
		secTable[j][2] = dx;
		secTable[j][3] = fl;
		secTable[j][4] = abs(dx)/abs(pow(temp,1.62));
		secTable[j][5] = C*pow(abs(dx),1.62);
		temp = dx;
		if (abs(dx) < xacc || f == 0.0) return secTable;
	}
	throw("Maximum number of iterations exceeded in rtsec");
}
...
template <class T>
MatDoub zriddrM(T &func, const Doub x1, const Doub x2, const Doub xacc) {
	const Int MAXIT=60;
	...
	if ((fl > 0.0 && fh < 0.0) || (fl < 0.0 && fh > 0.0)) {
		...
		for (Int j=0;j<MAXIT;j++) {
			...
			Doub fm=func(xm);
			Doub s=sqrt(fm*fm-fl*fh);
			if (s == 0.0) return ridTable;
			Doub xnew=xm+(xm-xl)*((fl >= fh ? 1.0 : -1.0)*fm/s);
			if (abs(xnew-ans) <= xacc) return ridTable;
			ans=xnew;
			Doub fnew=func(ans);
			if (fnew == 0.0) return ridTable;
			if (SIGN(fm,fnew) != fm) {
				xl=xm;
				fl=fm;
				xh=ans;
			...
			ridTable[j][2] = xh-xl;
			ridTable[j][3] = fl;
			if (abs(xh-xl) <= xacc) return ridTable;
		}
		throw("zriddr exceed maximum iterations");
	}
	else {
